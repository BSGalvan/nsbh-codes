...
# for other quantities as given in GWTC-2 for GW190426_152155
...
if __name__ == "__main__":
    ...
    FILE_NAME = "GW190426_152155.h5"
    FILE_PATH = f"{DATA_PATH}/{FILE_NAME}"

    # %% Read the posterior files

    with h5py.File(FILE_PATH, "r") as f:
        grp = f["C01:IMRPhenomPv2"]
    ...
    lambda_ns = 228 * np.ones_like(mass_bh)
    ...
    plt.scatter(
        mass_ratio[~disc_mask], spin_bh[~disc_mask], c="white", edgecolors="k", s=12
    )
    plt.xlabel(r"Mass Ratio, $\mathcal{Q}$")
    plt.ylabel(r"Black Hole Spin, $\chi_{BH}$")
    plt.title(r"GW190426\_152155 in the $\mathcal{Q}-\chi_{BH}$; High Spin Posterior")
    plt.tight_layout()
    plt.show()
