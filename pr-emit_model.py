...
def gaussian(theta, phi, theta_v):
    ...
    E_c = calc_E_kin_jet(1.4, 0.8) / (PI * theta_E**2)
    dE_dOmega = E_c * np.exp(-(theta/theta_E)**2)
    cos_alpha = (
                    np.cos(theta_v) * np.cos(theta) +
                    np.sin(theta_v) * np.sin(theta) * np.cos(phi)
                 )
...
def do_gauss_cutoff_integral(theta_v, cutoff_angle):
    ...
    ans, err, out_dict = nquad(gaussian,
...
E_kin_iso = 4 * PI * calc_E_kin_jet(0.2, 0.8) / \
            (PI * theta_E**2) * np.exp(-(view_angle/theta_E)**2)

E_iso = list()
cutoff = PI / 3
for idx, angle in enumerate(view_angle):
    print("Gauss Status: Currently doing ",
         view_angle[idx],
...
ax1.invert_yaxis()
l1 = ax1.loglog(np.degrees(view_angle), E_kin_iso / E_kin_iso[0],
                '--', color='gray', label="Kinetic", lw=4) 
l2 = ax1.loglog(np.degrees(view_angle), E_iso / E_iso[0],
                'r', label="Radiated", lw=4)
...
l3 = ax2.loglog(np.degrees(view_angle), lorentz,
                'b', label="Lorentz Factor", lw=4)
